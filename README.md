...
- For cyclic timers, when `Warp Speed` is enabled and the active phase unit is `s` or `Min`, elapsed runtime advances at 60x speed (1 real second = 60 countdown seconds) while the countdown format remains `MMM:SS`.
